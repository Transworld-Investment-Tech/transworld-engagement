...
// Designed birthday card — a committed brand asset, referenced by absolute URL
// (NEXT_PUBLIC_APP_URL, inlined by Next for both the browser preview and the
// server send). It is hosted rather than inlined so the in-app preview shows the
// real card and the send stays byte-identical to it. Clients that block remote
// images fall back to the alt text below, which carries the full warm message —
// so the greeting is complete and on-brand with or without images.
const APP_BASE = (
  process.env.NEXT_PUBLIC_APP_URL || "https://transworld-engagement.vercel.app"
).replace(/\/+$/, "");

const BIRTHDAY_CARD_SRC = `${APP_BASE}/birthday-card.jpg`;
...
// everywhere else.
export function birthdayCardHtml() {
  return `<div style="text-align:center;margin:20px 0 0;">
<img src="${BIRTHDAY_CARD_SRC}" alt="${escapeHtml(
    BIRTHDAY_CARD_ALT
