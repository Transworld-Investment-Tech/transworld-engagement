...
// Designed birthday card — a committed brand asset, hosted (not inlined, which
// would bloat every email past Gmail's clipping limit). The email references it
// by ABSOLUTE URL (NEXT_PUBLIC_APP_URL) because a mail client has no other way
// to resolve it; the in-app preview references it by a SAME-ORIGIN relative path
// (see renderGreeting/birthdayCardHtml) so it always loads from whatever host the
// app is on and never depends on the production asset or a deploy having
// propagated. Clients that block remote images fall back to the alt text below,
// which carries the full warm message — so the greeting is complete and on-brand
// with or without images.
const APP_BASE = (
  process.env.NEXT_PUBLIC_APP_URL || "https://transworld-engagement.vercel.app"
).replace(/\/+$/, "");

const BIRTHDAY_CARD_FILE = "/birthday-card.jpg";
const BIRTHDAY_CARD_ALT =
  "Happy Birthday from Transworld Investment & Securities Limited. On behalf of " +
  "everyone at the firm, we wish you good health, happiness, and prosperity in " +
  "the year ahead.";

// The hero card block, sized to the panel's inner width (600 − 2×32 padding).
// width="536" pins it for Outlook; the inline max-width:100% keeps it responsive
// everywhere else. `assetBase`: absolute origin for emails; pass "" from the
// in-app preview to load the card same-origin (a relative "/birthday-card.jpg").
export function birthdayCardHtml(assetBase) {
  const base = (assetBase == null ? APP_BASE : assetBase).replace(/\/+$/, "");
  const src = `${base}${BIRTHDAY_CARD_FILE}`;
  return `<div style="text-align:center;margin:20px 0 0;">
<img src="${src}" alt="${escapeHtml(
    BIRTHDAY_CARD_ALT
  )}" width="536" style="width:100%;max-width:536px;height:auto;display:block;margin:0 auto;border:1px solid #E4E2DB;border-radius:8px;">
</div>`;
}

// Full rendered email for one contact: returns { subject, html }.
// For a birthday greeting (type === "birthday") the designed card is appended
// below the (merge-rendered) salutation, so the client sees their name and then
// the card, which carries the message and the firm's sign-off.
// `opts.assetBase`: pass "" from the in-app preview so the card loads
// same-origin; omit it on the send path so the email gets the absolute URL.
export function renderGreeting(template, contact, opts = {}) {
  const subject = renderMergeTags(template.subject || "", contact);
  let bodyHtml = renderMergeTags(template.html_body || "", contact);
  if (template && template.type === "birthday") {
    bodyHtml += birthdayCardHtml(opts.assetBase);
  }
...
// --- Birthday-window predicates (Africa/Lagos) -----------------------------
// Shared by the dashboard counts and the contacts birthday filter so a card's
// number and the list it surfaces are always the same set.

// True if the birthday falls in the current calendar month (year ignored).
export function isBirthdayThisMonth(dobStr, today = lagosToday()) {
  const md = monthDay(dobStr);
  return !!md && md.month === today.month;
}

// True if the next occurrence of the birthday is within `days` days (year
// ignored). Today counts as day 0, so days=7 means today through six days out.
export function isBirthdayWithinDays(dobStr, days, today = lagosToday()) {
  const md = monthDay(dobStr);
  if (!md) return false;
  return daysUntil(today, md.month, md.day) < days;
}
