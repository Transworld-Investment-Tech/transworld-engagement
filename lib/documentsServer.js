...
import { formatLagos, shortRef, allSigned } from "@/lib/documents";
...
function drawFooter(page, fonts) {
  page.drawRectangle({ x: MARGIN, y: 70, width: A4.w - MARGIN * 2, height: 0.7, color: LINE });
  page.drawText(
    "Transworld Investment & Securities Limited · Lagos, Nigeria · Confidential",
    { x: MARGIN, y: 56, size: 8, font: fonts.sans, color: MUTED }
  );
}
...
// Builds the two appended pages (Execution + Certificate of Completion) onto the
// original PDF and returns the saved bytes. Field positions used for each
// signature on the Execution page are returned too, so signature_fields can be
// persisted (a clean upgrade path to a drag-to-place editor later).
//
// pdf-lib ships standard fonts only; Georgia cannot be embedded without the
// font file, so Times Roman is used as the serif on these generated pages —
// the closest standard serif to the house Georgia.
export async function assembleSignedPdf({ originalBytes, document, signatories }) {
  ...
  const placedFields = [];
  const execPageIndex = pdf.getPageCount(); // 0-based index of the exec page

  // ---- Execution page ------------------------------------------------------
  const exec = pdf.addPage([A4.w, A4.h]);
  let y = drawHeader(exec, fonts, "Execution Page");
  ...
  for (const s of ordered) {
    ...
    placedFields.push({
      ...
      page: execPageIndex + 1, // 1-based for the schema
      pos_x: MARGIN,
      pos_y: boxY,
      width: boxW,
      height: boxH,
    ...
    if (s.signature_type === "drawn") {
      const bytes = decodePng(s.signature_data);
      if (bytes) {
        try {
          const png = await pdf.embedPng(bytes);
          const scale = Math.min((boxW - 16) / png.width, (boxH - 12) / png.height);
          exec.drawImage(png, {
            x: MARGIN + 8,
            y: boxY + (boxH - png.height * scale) / 2,
            width: png.width * scale,
            height: png.height * scale,
          });
        } catch {
          /* fall through to a typed fallback below */
        }
      }
    } else if (s.signature_type === "typed") {
      exec.drawText(s.signature_data || s.name, {
        x: MARGIN + 12,
        y: boxY + 18,
        size: 24,
    ...
    exec.drawText(`Signed electronically on ${formatLagos(s.signed_at)}`, {
      x: MARGIN,
      y: boxY - 16,
      size: 9,
      font: fonts.serif,
  ...
  drawFooter(exec, fonts);

  // ---- Certificate of Completion ------------------------------------------
  const cert = pdf.addPage([A4.w, A4.h]);
  let cy = drawHeader(cert, fonts, "Certificate of Completion");
  cy = drawWrapped(
    cert,
  ...
  const r2 = cy;
  const l2e = labelValue(cert, fonts, MARGIN, r2, "Completed", formatLagos(document.completed_at || new Date()));
  const r2e = labelValue(cert, fonts, col2, r2, "Parties", String(ordered.length));
  ...
  for (const s of ordered) {
    ...
    const rows = [
      ["Email", s.email],
      ["Verification", method],
      ["Consent to sign electronically", s.consent_given ? "Given" : "Not recorded"],
      ["Signed at", formatLagos(s.signed_at)],
  ...
  // integrity fingerprint — the ORIGINAL document hash (a file cannot contain
  // its own final hash; the final-PDF hash is stored separately in the DB).
  ...
  cy = drawWrapped(
    cert,
    "This is a unique fingerprint of the uploaded document. If a single byte of the original changes, this value changes — so it can be used to prove the signed instrument matches what was presented.",
    { x: MARGIN, y: cy, font: fonts.serif, size: 9, color: MUTED, maxWidth: colW, lineHeight: 12 }
  );
  drawFooter(cert, fonts);

  const bytes = await pdf.save();
  return { bytes, placedFields, originalHash };
}
...
export async function finalizeIfComplete(documentId) {
  ...
  const { bytes, placedFields } = await assembleSignedPdf({
    originalBytes,
  ...
  // Persist the computed signature-field positions (idempotent: clear + insert).
  await supabase.from("signature_fields").delete().eq("document_id", documentId);
  if (placedFields.length) {
    await supabase
      .from("signature_fields")
      .insert(placedFields.map((f) => ({ ...f, document_id: documentId })));
  }
  ...
  await logEvent({
    ...
    actor: "system",
    metadata: { final_sha256: finalHash },
  });
