...
// A safe download filename for the signed PDF, derived from the title.
...
// The signing-request email a client receives with their one-time link.
export function signingRequestEmail({ signerName, documentTitle, signUrl, expiresLabel }) {
  const subject = `Document for your signature — ${documentTitle}`;
  const bodyHtml = `
<p style="margin:0 0 16px;">Dear ${escapeHtml(signerName)},</p>
<p style="margin:0 0 16px;">Transworld Investment &amp; Securities Limited has prepared a document that requires your signature: <strong>${escapeHtml(
    documentTitle
  )}</strong>.</p>
<p style="margin:0 0 16px;">Please review and sign it using your secure personal link below. For your protection, you will be asked to enter a one-time code that we send to this email address before you sign.</p>
<p style="margin:24px 0;text-align:center;">
  <a href="${escapeHtml(signUrl)}" style="display:inline-block;background:#0B1F3A;color:#ffffff;text-decoration:none;font-family:Arial,Helvetica,sans-serif;font-size:14px;font-weight:bold;padding:13px 26px;border-radius:8px;">Review &amp; sign the document</a>
</p>
...
<p style="margin:24px 0 0;">Warm regards,<br>The Transworld Team</p>`;
  return {
    subject,
    html: renderEmailHtml({ subject, bodyHtml, preheader: `Please review and sign ${documentTitle}.` }),
  };
}
...
// Confirmation to the client once the fully executed document is complete.
// The fully signed PDF is attached to this email.
export function completionEmail({ signerName, documentTitle }) {
  const subject = `Completed: ${documentTitle}`;
  const bodyHtml = `
<p style="margin:0 0 16px;">Dear ${escapeHtml(signerName)},</p>
<p style="margin:0 0 16px;">Thank you. <strong>${escapeHtml(
    documentTitle
  )}</strong> has now been fully executed by all parties, including Transworld Investment &amp; Securities Limited.</p>
<p style="margin:0 0 16px;">A fully signed copy is <strong>attached to this email</strong> for your records, including an execution page and a certificate of completion with a tamper-evidence checkmark. Please keep it safe.</p>
<p style="margin:24px 0 0;">Warm regards,<br>The Transworld Team</p>`;
  return {
    subject,
    html: renderEmailHtml({ subject, bodyHtml, preheader: `${documentTitle} is fully signed — copy attached.` }),
  };
}
